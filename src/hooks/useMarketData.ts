...
export function useMarketData() {
  ...
  useEffect(() => {
    const initial: NewsItem[] = newsPool.slice(0, 12).map((n, i) => ({
      ...n,
      id: `news-${i}`,
      time: new Date(Date.now() - i * 180000).toLocaleTimeString("en-US", { hour: "2-digit", minute: "2-digit", second: "2-digit", hour12: false }),
    }));
    setNews(initial);
    newsIndexRef.current = 12;
  }, []);

  // Update stocks every 1.2s
  useEffect(() => {
    const interval = setInterval(() => {
      setStocks(prev => prev.map(stock => {
        const newPrice = randomFluctuation(stock.price, 0.25);
  ...
  // Update indices every 2.5s
  useEffect(() => {
    const interval = setInterval(() => {
      setIndices(prev => prev.map(idx => {
        const newValue = randomFluctuation(idx.value, 0.08);
  ...
  // Update forex every 1.5s
  useEffect(() => {
    const interval = setInterval(() => {
      setForex(prev => prev.map(f => {
        const newRate = randomFluctuation(f.rate, 0.03);
  ...
  // Update crypto every 2s
  useEffect(() => {
    const interval = setInterval(() => {
      setCrypto(prev => prev.map(c => {
        const newPrice = randomFluctuation(c.price, 0.4);
        const newChange = Math.round((newPrice - (c.price - c.change)) * 100) / 100;
        return { ...c, price: newPrice, change: newChange, changePercent: Math.round((newChange / (newPrice - newChange)) * 10000) / 100 };
      }));
    }, 2000);
  ...
  return { stocks, indices, forex, commodities, bonds, crypto, news, selectedStock, setSelectedStock };
}
